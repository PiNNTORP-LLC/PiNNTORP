...
public class AuthHelper {
    ...
    public static boolean verifyPassword(String rawPassword, String base64Salt, String base64Hash) {
        String testHash = hashPassword(rawPassword, base64Salt);
        return testHash.equals(base64Hash);
    }
    ...
    public static String validateJWTAndGetUsername(String token) {
        ...
        try {
            ...
            String expectedSignatureB64 = Base64.getUrlEncoder().withoutPadding()
                    .encodeToString(expectedSignatureBytes);

            if (!signatureB64.equals(expectedSignatureB64)) {
                return null;
            }

            // Decode payload to extract subject (username)
            String payloadJson = new String(Base64.getUrlDecoder().decode(payloadB64), "UTF-8");
