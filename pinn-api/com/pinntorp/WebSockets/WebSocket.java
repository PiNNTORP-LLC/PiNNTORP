...
public class WebSocket {
     ...
     * @throws NoSuchAlgorithmException if the SHA-1 algorithm isn't available for
    ...
    public WebSocket(Socket socket, java.util.Map<String, String> headers)
        ...
        // Check if we have the key from headers
        String keyString = headers.get("Sec-WebSocket-Key");
    ...
    public Message receive() {
        ...
        try {
            ...
            System.out.println("Encountered " + e.getClass().getName()
                    + " while attempting to receive string over websocket.\n" + e);
        }
     ...
     * @param maskingKey the 4 byte masking key to mask the payload with if masked
     *                   is set to true
     * @param masked     the masked flag, which should only be set to true for
     *                   client to server frames
     * @param opcode     the frame <a href=
     *                   "https://en.wikipedia.org/wiki/WebSocket#Opcodes">opcode</a>
     */
    public void send(byte[] message, byte[] maskingKey, boolean masked, int opcode) {
        ...
        try {
            ...
            System.out.println("Encountered " + e.getClass().getName()
                    + " while attempting to send message over websocket.\n" + e);
        }
    }
    ...
    /**
     * Send a close request to the client, closing the WebSocket upon receiving the
     * close response.
     * Alternatively, send close response and close the WebSocket if clientInitiated
     * is set to true.
     * 
     * @param clientInitiated Set to true if the first close frame was received from
     *                        the client, indicating client requested close
     * @throws IOException If an I/O error occurs while closing the underlying TCP
     *                     socket.
     */
    public void close(boolean clientInitiated) throws IOException {
        // Check if close was initiated by client
        if (clientInitiated) {
            // Close was initiated by client, send close response with status code 1000
            // "Normal closure"
